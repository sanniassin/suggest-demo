...
var onSearchItemsLoaded = function onSearchItemsLoaded(res) {
  loadingIcon.hidden = true;
  suggestHint.hidden = true;
  searchResults.hidden = false;

  if (!res.length) {
    searchResultsLabel.innerHTML = "Nothing found for <strong>".concat(escapeHtml(input.value), "</strong>");
    return;
  }

  searchResultsLabel.innerHTML = "Results for <strong>".concat(escapeHtml(input.value), "</strong>:");
  var searchResultsItemsHtml = res.map(function (item) {
    return "<div class=\"search-item\">".concat(escapeHtml(item), "</div>");
  }).join('');
  searchResultsItems.innerHTML = searchResultsItemsHtml;
};

var loadSearchItems = function loadSearchItems() {
  var query = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : input.value;
  ...
  (0, _processQuery.getSearchItems)(query).then(function (searchRes) {
...
var onInputChange = function onInputChange() {
  ...
  (0, _processQuery.getSuggestItems)(value).then(function (res) {
    ...
    var suggestHtml = res.map(function (item) {
      return "<div class=\"input-suggest__item\">".concat(escapeHtml(item), "</div>");
    }).join('');
    ...
    suggestItems.forEach(function (item, index) {
      var onSuggestItemMouseEnter = function onSuggestItemMouseEnter() {
        suggestItems.forEach(function (_item) {
          _item.classList.remove('input-suggest__item--hovered');
        });
        item.classList.add('input-suggest__item--hovered');
      };

      var onSuggestItemSelect = function onSuggestItemSelect() {
        hideSuggest();
        value = res[index];
        input.value = res[index];
        loadingIcon.hidden = false;
        (0, _processQuery.getSearchItems)(input.value).then(function (searchRes) {
...
var onInputKeyDown = function onInputKeyDown(event) {
  if (event.keyCode !== _keys.KEY_ENTER) {
    return;
  }
...
/***/ })
/******/ ]);
//# sourceMappingURL=vanilla-app.js.map